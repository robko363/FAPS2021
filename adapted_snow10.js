//as used in Snow10 (https://github.com/beardog108/snow10)
/*
CryptoJS v3.1.2
...
*/
//removed white steg functionality
...
//adjusted function - added parameters to substitute previous web element calls
function go(mode, input, encryption, password) {
	var output = '';
	...
	if (mode == 'encode')
	{
		...
		if (encryption)
		{
			input = CryptoJS.AES.encrypt(input, password).toString();
		}
		// convert result to binary
		output = textToBin(encodeURIComponent(input));
		return replaceAll(replaceAll(output.toString(), "1", z_one), "0", z_zero);
	}
	else
	{
	    var sanitized = "";
	    ...
	    for (var i = 0; i < split.length; i++)
            if(split[i] == z_one || split[i] == z_zero)
                sanitized = sanitized + split[i]
		var output = decodeURIComponent(binToText(sanitized));
		if (output === 'undefined'){
			throw new Error('Could not decode message. Maybe there wasn\'t one?');
		}
		if (encryption)
		{
			output = CryptoJS.AES.decrypt(output, password).toString(CryptoJS.enc.Utf8);
		}
		return output.toString();
	}
}

//added require for IO
const fs = require('fs');

//added function: encoding file with name
/**
    Hide file with Snow10
    ...
    @returns {String} stego (String)
**/
function hide_file(file_name, encryption, password) {
    ...
    var add_name = 'snow' + file_name + ':';
    var data_string = '';                                                               //convert to string
    for (var i=0; i<file_data.length; i++) {
        data_string += String.fromCharCode(file_data[i]);
    }
    var stego_with_name = go('encode', add_name+data_string, encryption, password);
    return stego_with_name;
}

//added function: decode file with name
/**
    Reveal snow10 encoded file and save it
    @param stego {String} text with hidden file (String)
    @param encryption {boolean} whether to use encryption or not (boolean)
    @param password {String} password to be used if encryption is selected (String)
**/
function reveal_file(stego, encryption, password) {
    var decoded = go('decode', stego, encryption, password);
